...
static constexpr const int G_MAXSIZE = 0x7FFFFFFF;
#define g_return_val_if_fail(_cond, _val) if ((_cond) == false) return _val;
#define g_assert(_cond) do {} while(false)
...
* Since: 2.12
*/
std::string g_base64_encode(const guchar *data, gsize         len)
{
	std::string outStr;
	gchar *out;
	gint state = 0, outlen;
